...
void main_screen()
{
  ...
  client.publish("/Medibox/time", current_time.c_str()); // send the current time to web via MQTT.
  ...
  if (nearestAlarmIndex != -1)
  {
    ...
    String next_alarm = String(timeArray[nearestAlarmIndex].hours + ":" + timeArray[nearestAlarmIndex].minutes);
    client.publish("/Medibox/next_alarm", next_alarm.c_str());
  }
  else
  {
...
void displaySetAlarmMenu(int alarm_num)
{
  ...
  while (position != 0)
  {
    ...
    else if (B_dir == EXIT_SAVE)
    {
      ...
      /*
          EEPROM mem. addr   |      alarm no.    |   data type
          ---------------------------------------------------------
                  0          |        1          |      hours
                  1          |        1          |      mins
                  2          |        2          |      hours
                  ...
                  5          |        3          |      mins
      */
      EEPROM.write((alarm_num * 2) - 2, timeArray[alarm_num - 1].hours);
      EEPROM.write((alarm_num * 2) - 1, timeArray[alarm_num - 1].minutes);
...
void enable_disable_alarms_menu()
{
  ...
  while (true)
  {
    ...
    else if (B_dir == MENU_NEXT)
    {
      ...
      { // "Disable all alarms" option selected
